...
if [ ! -f ~/Desktop/QIIME2/$1/"$1".fastq ]; then
	echo -e "FastQ file not found in ~/Desktop/qiime2/$1\n"
	exit
fi
#
echo "Okay, doing step 1. Give me 5 minutes."
extract_barcodes.py -f $HOME/Desktop/QIIME2/$1/$1.fastq -o ~/Desktop/QIIME2/$1/emp -l 15 -c barcode_single_end -m ~/Desktop/QIIME2/$1/"$1"map.txt
#Then to rename reads.fastq to sequences.fastq as per EMP protocol
mv $HOME/Desktop/QIIME2/$1/emp/reads.fastq $HOME/Desktop/QIIME2/$1/emp/sequences.fastq
#Then to gzip both new files as per EMP protocol
gzip $HOME/Desktop/QIIME2/$1/emp/sequences.fastq
gzip $HOME/Desktop/QIIME2/$1/emp/barcodes.fastq
#
